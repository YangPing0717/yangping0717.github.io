...
$.extend({
	msgDialog: function(param) {
		...
		param.title = typeof param.title === 'string' ? param.title : '錯誤';
		param.content = typeof param.content === 'string' ? param.content : '';
		param.confirmTxt = typeof param.confirmTxt === 'string' ? param.confirmTxt : '確認';
		var dialog = new BootstrapDialog({
			animate: false,
			title: param.title,
			message: '<div class="lb_text">' + param.content + '</div>',
			buttons: [{
				label: param.confirmTxt,
				cssClass: 'l_btn l_btn_small_f l_btn_mr l_btn_dark',
				action: function(_dialog) {
					if (typeof param.beforeClose === 'function') {
						eval(param.beforeClose)(_dialog);
					}
					_dialog.close();
				}
			}],
			onshow: function(_dialog) {
				if (typeof param.beforeOpen === 'function') {
					eval(param.beforeOpen);
				}
			},
			onshown: function(_dialog) {
				if (typeof param.afterOpen === 'function') {
					eval(param.afterOpen);
				}				
			},
			onhidden: function(_dialog) {
				if (typeof param.afterClose === 'function') {
					eval(param.afterClose)(_dialog);
				}				
			}
		});
		dialog.realize();
		dialog.getModalHeader().addClass('modal-header2');
		dialog.getModalHeader().find('button.close').addClass('lb_close');
		dialog.getModalFooter().addClass('modal-footer2');
		dialog.open();
		return dialog;
	},
	ynDialog: function(param) {
		if (typeof param !== 'object') { return false;}
		param.title = typeof param.title === 'string' ? param.title : '提示';
		param.content = typeof param.content === 'string' ? param.content : '';
		param.yesTxt = typeof param.yesTxt === 'string' ? param.yesTxt : '確認';
		param.noTxt = typeof param.noTxt === 'string' ? param.noTxt : '取消';
		param.btnReverse = typeof param.btnReverse === 'boolean' ? param.btnReverse : false;
		var btn = [{
			label: param.noTxt,
			action: function(_dialog) {
				if (typeof param.beforeClose === 'function') {
				...
				_dialog.close();
				if (typeof param.sayNo === 'function') {
					eval(param.sayNo)(_dialog);
				}
			}
		}, {
			label: param.yesTxt,
			action: function(_dialog) {
				...
				_dialog.close();
				if (typeof param.sayYes === 'function') {
					eval(param.sayYes)(_dialog);
				}
			}
		}];
		if (param.btnReverse) {
		...
		btn[0].cssClass = 'l_btn l_btn_small_f l_btn_mr';
		btn[1].cssClass = 'l_btn l_btn_small_f l_btn_mr l_btn_dark';
		var dialog = new BootstrapDialog({
			animate: false,
			title: param.title,
			message: '<div class="lb_text">' + param.content + '</div>',
